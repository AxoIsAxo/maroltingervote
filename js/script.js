...
const firebaseConfig = {
  apiKey: "AIzaSyDoo957xiZB2heugMso-C6oS1jSjciLUq0", // Replace with your actual API key
  authDomain: "maroltingervote.firebaseapp.com",
  projectId: "maroltingervote",
  storageBucket: "maroltingervote.appspot.com", // Usually .appspot.com for this config
  messagingSenderId: "536390639151",
  appId: "1:536390639151:web:3f96ba960920552ecdb0da",
  measurementId: "G-4TRS7E6NGK" // Optional, remove if not using Analytics
};
...
try {
    ...
    const errorDisplayInit = () => {
        ...
        if (errorTarget === document.body) errorTarget.insertBefore(errorMsgElement, errorTarget.firstChild);
        else errorTarget.appendChild(errorMsgElement);
    };
...
const PREDEFINED_ITEMS_CONFIG = [
    ...
    { id: "DOG", initialLikes: 0, initialDislikes: 0 },
];
...
function initializeMainAppLogic() {
    ...
    // ... (all your other getElementById calls - I'll abbreviate here for this example)
    ...
    if (!loginButton || !authContainer /* ... other checks ... */ || !verificationMessageP) {
    ...
    onAuthStateChanged(auth, async (user) => {
        // ... (This entire onAuthStateChanged block is IDENTICAL to the last full script version)
...
// (These are IDENTICAL to the last full script version)
async function initializeLikingSystem() { console.log("%c--- initializeLikingSystem CALLED ---", "color: purple; font-weight: bold;"); if (auth.currentUser && auth.currentUser.emailVerified) { currentUserId = auth.currentUser.uid; } else { if(mainContentWrapper) mainContentWrapper.style.display = 'none'; if(authRequiredMessage) authRequiredMessage.style.display = 'block'; return; } itemListeners.forEach(unsubscribe => unsubscribe()); itemListeners = []; try { await loadUserVotes(); } catch (e) { userVotes = {}; } const mainContainer = document.querySelector('.main'); if (!mainContainer) { console.error("Liking Sys: '.main' container NOT FOUND."); return; } mainContainer.innerHTML = ''; itemsData = {}; for (const pItemConfig of PREDEFINED_ITEMS_CONFIG) { const itemId = pItemConfig.id; const h2Prefix = `${itemId}: `; const firestoreItemData = await ensureItemDocExists(itemId, pItemConfig); if (!firestoreItemData) continue; const boxElement = document.createElement('div'); boxElement.className = 'box'; boxElement.dataset.itemId = itemId; const h2 = document.createElement('h2'); const voteControls = document.createElement('div'); voteControls.className = 'vote-controls'; voteControls.innerHTML = `<button class="like-btn">👍<span class="likes-count">(0)</span></button><button class="dislike-btn">👎<span class="dislikes-count">(0)</span></button>`; boxElement.appendChild(h2); boxElement.appendChild(voteControls); mainContainer.appendChild(boxElement); itemsData[itemId] = { likes: firestoreItemData.likes, dislikes: firestoreItemData.dislikes, internalScore: calculateInternalScore(firestoreItemData.likes, firestoreItemData.dislikes), h2Prefix: h2Prefix, originalOrder: PREDEFINED_ITEMS_CONFIG.findIndex(i => i.id === itemId), element: boxElement }; voteControls.querySelector('.like-btn').addEventListener('click', () => handleVote(itemId, 'like')); voteControls.querySelector('.dislike-btn').addEventListener('click', () => handleVote(itemId, 'dislike')); updateBoxDisplay(itemId); const itemRef = doc(db, 'items', itemId); const unsubscribe = onSnapshot(itemRef, (docSnap) => { if (docSnap.exists()) { const d = docSnap.data(); if (itemsData[itemId] && (itemsData[itemId].likes!==d.likes || itemsData[itemId].dislikes!==d.dislikes)) { itemsData[itemId].likes=d.likes; itemsData[itemId].dislikes=d.dislikes; updateBoxDisplay(itemId); renderSortedBoxes(); } } }, e => console.error(`Listen err ${itemId}:`,e)); itemListeners.push(unsubscribe); } renderSortedBoxes(); }
